...
def onRead():
    try:
        ...
        if data[0][:2] == '06':  # Якщо це значення вологості (06)
            humi = float(data[0][2:])  # Отримуємо значення вологості
            if 0 <= humi <= 100:  # Перевіряємо, щоб вологість була в межах від 0 до 100%
                ui.humidityBar.setValue(int(humi))  # Перетворюємо на ціле число
            else:
                print(f"Неправильне значення вологості: {humi}")
            ui.humiB.setStyleSheet("background-color: green; color: white;")
